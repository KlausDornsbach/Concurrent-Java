...
public class Main {
    ...
    private @Nonnull Thread createSender(int idx, @Nonnull MessageBroker broker) {
        Thread t = new Thread(() -> {
            boolean end = false;
            for (int i = 0; !end; i++) {
                if (!broker.caixaPostal.containsKey("r" + idx)) {
                    String address = "r" + idx;
                    LinkedList<Message> list = new LinkedList<>();
                    broker.caixaPostal.put(address, list);
                    ReentrantLock l = new ReentrantLock();
                    broker.locks.put(address, l);
                    Semaphore s = new Semaphore(0);
                    broker.semaphoreSend.put(address, s);
                    broker.semaphoreReceive.put(address, s);
                }
                if (!broker.caixaPostal.containsKey("s" + idx)) {
                    String address = "s" + idx;
                    LinkedList<Message> list = new LinkedList<>();
                    broker.caixaPostal.put(address, list);
                    ReentrantLock l = new ReentrantLock();
                    broker.locks.put(address, l);
